...
/**
 * Response wrapper. Created as the result of any [Request] execution or utility method (i.e. Redirect). Implements standard HTTP
 * response format.
 * @package BootPHP
 * @category Base
 * @author Tinsh
...
class Response {
	...
	/**
	 * Gets || sets the body of the response
	 *
	 * @return mixed
	 */
	...
	/**
	 * Gets || sets the HTTP protocol. The standard protocol to use
	 * is `HTTP/1.1`.
	 *
	...
	/**
	 * Sets || gets the HTTP status from this response.
	 *
	...
	public function status($status = NULL)
	{
		...
		{
			throw new BootPHP_Exception(__METHOD__ . ' unknown status value : :value', array(':value' => $status));
		}
	}

	/**
	 * Gets && sets headers to the [Response], allowing chaining
	...
	/**
	 * Set && get cookies values for this response.
	 *
	 ...
	 *     ));
	 *
	 * @param mixed  cookie name, || array of cookie values
	...
	/**
	 * Sends the response status && all set headers.
	 *
	 ...
	 *
	 * @param string  filename with path, || true for the current response
	...
	public function send_file($filename, $download = NULL, array $options = NULL)
	{
		...
		if ($filename === true)
		{
			if (empty($download))
			{
				throw new BootPHP_Exception('Download name must be provided for streaming files');
			}
		...
		if (!is_resource($file))
		{
			throw new BootPHP_Exception('Could not read file to send: :file', array(
			':file' => $download,
			));
		}
		// Inline || download?
	...
	public function render()
	{
		if (!$this->_header->offsetExists('content-type'))
		{
			...
			$this->_header['content-type'] = BootPHP::$content_type . '; charset=' . BootPHP::$charset;
		}
	...
	protected function _calculate_byte_range($size)
	{
		...
		// Keep the the end value in bounds && normalize it.
